...
def randslt(SV, ec, budsum, X_train=None, y_train=None):
    ...
    optsum = 0
    for idx in idx_lst:
        if models.IrisHistoryInfo.objects.filter(id=idx):
            status_now = models.IrisHistoryInfo.objects.get(id=idx).status + 1
            com_now = models.IrisHistoryInfo.objects.get(id=idx).compensation + ec[idx]
            print(ec[idx])
            models.IrisHistoryInfo.objects.filter(id=idx).update(status=status_now, compensation=com_now)
        else:
            models.IrisHistoryInfo.objects.create(id=idx, status=1, compensation=ec[idx])
...
def randslt2(SV, ec, budsum, X_train=None, y_train=None):
    idx_lst = np.arange(len(SV))
    np.random.shuffle(idx_lst)
    idx_subset = []
    X_subset = []
...
def alllist(SV, ec, budsum, X_train=None, y_train=None):
    idx_lst = np.arange(len(SV))
    alllist = PowerSetsBinary(idx_lst)
    max = 0.0
    for list in alllist:
        X_subset = []
        y_subset = []
        ...
        for idx in list:
            ...
            X_subset.append(X_train[idx])
            y_subset.append(y_train[idx])
            if (float(ec_sum) > float(budsum)):
                ec_sum -= ec[idx]
                break
        if (ec_sum <= float(budsum) and SV_sum > float(max)):
            max = SV_sum
            idx_subset = list
    print("all",max)
    return SV_sum, X_subset, y_subset
...
def greedy2(SV, ec, budsum, X_train, y_train):
    sorted_ratio = get_ratio(SV, ec)
...
def DPSV(SV, ec, budsum, X_train, y_train):
    ...
    for idx in path:
        if models.IrisHistoryInfo.objects.filter(id=idx):
            status_now = models.IrisHistoryInfo.objects.get(id=idx).status + 1
            com_now = models.IrisHistoryInfo.objects.get(id=idx).compensation + ec[idx-1]
            models.IrisHistoryInfo.objects.filter(id=idx).update(status=status_now, compensation=com_now)
        else:
            models.IrisHistoryInfo.objects.create(id=idx, status=1, compensation=ec[idx-1])
...
def DPSV2(SV, ec, budsum, X_train, y_train):
    bcec = [ec[i] for i in range(len(SV))]
    a, bcec = approximate(bcec)
    budsum *= a
...
def main(SV_train, X_train, X_test, y_train, y_test, eps_list, budeps, bp, ps, mode='GREEDY', portion=1):
    ...
    for eps in eps_list:
        ...
        if mode == 'GREEDY':
            ...
            optsum, X_train_subset, y_train_subset = randslt2(SV_train, ecs, budeps, X_train, y_train)
...
def main2(SV_train, X_train, X_test, y_train, y_test, eps_list, budeps, bp, ps, mode='GREEDY', portion=1):
    SV_dic = {}
    for i in range(len(SV_train)):
        SV_dic[i] = SV_train[i]
    sorted_dic = sorted(SV_dic.items(), key=lambda x: x[1], reverse=True)
    ...
    for eps in eps_list:
        ...
        if mode == 'GREEDY':
            ...
            optsum, X_train_subset, y_train_subset = greedy2(SV_train, ecs, budeps, X_train, y_train)
            end = time.perf_counter()
            print("运行时间：", end - start)
        elif mode == 'DP':
            start = time.perf_counter()
            optsum, X_train_subset, y_train_subset = DPSV2(SV_train, ecs, budeps, X_train, y_train)
            end = time.perf_counter()
            print("运行时间：", end-start)
        elif mode == 'GUE-GRD':
            optsum, X_train_subset, y_train_subset = guess_greedy(SV_train, ecs, budeps, X_train, y_train)
            ...
            start = time.perf_counter()
            optsum, X_train_subset, y_train_subset = alllist(SV_train, ecs, budeps, X_train, y_train)
            end = time.perf_counter()
            print("运行时间：", end - start)
        else:
            start = time.perf_counter()
...
def loadChess_(valid_idx=None):
    ...
    train_cancer = np.array([list(i) for i in models.TrainChess.objects.all().values_list()])
    test_cancer = np.array([list(i) for i in models.TestChess.objects.all().values_list()])
