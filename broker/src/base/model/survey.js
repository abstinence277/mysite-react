...
import OptPricePop from './optprivepop';
...
export default class Survey extends Component {
    ...
    render() {
        ...
        const optPrice = this.getSample(maxRevenue).map((row, index) => (
            <tr key={index}>
                <td style={{padding: '0 2rem','height':'4rem'}}>{"M" + (index+1)}</td>
                {Array.from(row).map((col, i) => (
                    <td style={{padding: '0 4rem'}} key={i}>{col}</td>
                ))}
            </tr>
        ));
