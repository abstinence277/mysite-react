...
export default class Design extends Component {
    ...
    render() {
        ...
        const sampleData = this.getSample(ampRes).map((row, index) => (
            <tr key={index}>
                {column.map((col, i) => (
                    <td style={{padding: '0 5.2rem',height:'3rem'}} key={i}>{row[col.dataIndex]}</td>
                ))}
            </tr>
        ));
        ...
        return (
            <div>
                ...
                <div style={{marginTop:'.5rem'}}>
                    ...
                    <div style={{width:'60%',marginLeft:'6rem'}}>
                ...
                <div style={{marginTop:'.5rem'}}>
                    <span style={{width:'10rem',float:'left', textAlign:'left'}}>训练模型：</span>
                    <div style={{width:'2rem',marginLeft:'12rem'}}>
                        <Switch className="switchBroker" size="small" loading={loadingAlg} disabled={isDisabled}
                        checked={checked} onChange={this.calAMP}/>
                    </div>
                </div>
                <div style={{height:'20rem',width:'100%',marginTop:'3rem'}}>
                    <div className="tb tb_left" style={{width:'63rem',height:'15rem'}}>
                        <table>
                            <thead>
                                <tr className="tb_title" >
                                    {column.map((col, index) => (
                                        <td style={{padding: '0 5.2rem',height:'3rem'}} key={index}>{col.title}</td>
                                    ))}
                                </tr>
                            </thead>
                            <tbody>
