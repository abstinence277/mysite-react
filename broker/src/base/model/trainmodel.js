...
import ModelPopup from './modelpopop';
...
export default class TrainedModel extends Component {
    ...
    render() {
        ...
        const { modelData,dataset } = this.props

        //页面上展示的modelData
        const sampleData = this.getSample(modelData).map((row, index) => (
            <tr style={{padding: '0 2rem','height':'2rem'}} key={index}>
                {column.map((col, i) => (
        ...
        const modal = this.state.showModal ? (
            <Modal>
                <div className="modal">
                    <ModelPopup modelData={modelData} dataset={dataset} closeModal={this.handleHide}/>
                </div>
            </Modal>
        ) : null;
        ...
        return (
            <div>
                <div className="tb tb_left" style={{width:'63.9rem',height:'10rem'}}>
					<table>
                        <thead>
                            <tr style={{padding: '0 2rem','height':'2rem'}} className="tb_title">
